...
Four-stage flow:
  ...
  3. Auto-fix — priority chain: redundancy removal → language unification →
     sentence compression → bullet restructure → font micro-adjust
        ...
        for s in shapes:
            ...
            if matches:
                issues.append(NarrativeIssue(
                    ...
                    category="lang_mix",
                    message=f"Mixed-language terminology: {', '.join(set(m.lower() for m in matches))}",
                    shape_name=getattr(s, "name", ""),
                    suggestion="Informational only; established proper nouns may stay in their original form.",
                ))
        return issues
        ...
        for s in shapes:
            ...
            if limit and char_count > limit * 1.3:
                issues.append(NarrativeIssue(
                    ...
                    category="density",
                    message=f"Text density too high: {char_count} chars in {box_h_in:.2f}\" tall box (recommended ≤ {limit})",
                    shape_name=getattr(s, "name", ""),
                    suggestion="Trim the copy or split the content across slides.",
                ))
        return issues
                    ...
                    if run.font.size and run.font.size.pt >= 20:
                        ...
                        if len(text) > ACTION_TITLE_MAX_CHARS:
                            issues.append(NarrativeIssue(
                                ...
                                shape_name=getattr(s, "name", ""),
                                suggestion="Shorten the title; move supporting detail to a subtitle or the body.",
                            ))
...
#   1. Remove redundancy — strip weak hedging and filler phrasing
#   2. Unify language — replace inconsistent jargon (no-op for pure English)
#   3. Compress sentences — collapse verbose constructions
...
class AutoFixPipeline:
    ...
    def _apply_fixes(self, overflow_errors: List[QAIssue], verbose: bool) -> int:
        ...
        for err in overflow_errors:
            ...
            if fixed and self._check_fits(tf, box_w, box_h):
                fixes_count += 1
                if verbose:
                    print(f"    S{err.slide_num} [{err.shape_name}] 📐 sentence compressed → fits")
                continue
            ...
            if fixed and self._check_fits(tf, box_w, box_h):
                fixes_count += 1
                if verbose:
                    print(f"    S{err.slide_num} [{err.shape_name}] 🔄 bullets restructured → fits")
                continue
            ...
            if fixed:
                fixes_count += 1
                if verbose:
                    print(f"    S{err.slide_num} [{err.shape_name}] 🔠 font adjusted → fits")
                continue

            if verbose:
                print(f"    S{err.slide_num} [{err.shape_name}] ⚠️  no strategy resolved the overflow")
    ...
    def _harmonize_peer_fonts(self, verbose: bool) -> int:
        ...
        if total_adjusted > 0:
            ...
            if verbose:
                print("  ✅ All peer-group font sizes already consistent.")
